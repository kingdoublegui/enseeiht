function [p,nurbs_cor] = nrbplot (nurbs, subd, color, varargin)
% 
...
nargs = nargin;
if nargs < 3
  error ('Need a NURBS to plot and the number of subdivisions and the color!');
elseif rem(nargs+3,3)
...
if (iscell (nurbs.knots))
 ...
 elseif (size (nurbs.knots,2) == 3) % plot the boundaries of a NURBS volume
  ...
  if (any (nurbs.coefs(3,:)))
    % 3D curve
    nurbs_cor = plot3 (p(1,:), p(2,:), p(3,:),color); 
    grid on;
  else
    % 2D curve
    nurbs_cor = plot (p(1,:), p(2,:),color);
  end
end
axis equal;
