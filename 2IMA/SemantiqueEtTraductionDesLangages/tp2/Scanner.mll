...
rule scanner = parse
  ...
  | '*'
      {
      StarToken :: (scanner lexbuf)
    }
  | '/'
      {
      SlashToken :: (scanner lexbuf)
    }
  | "&&"
      {
      AndToken :: (scanner lexbuf)
    }
  | "||"
      {
      OrToken :: (scanner lexbuf)
    }
  | '!'
      {
      ReadToken :: (scanner lexbuf)
    }
  | "ref"
      {
      RefToken :: (scanner lexbuf)
    }
  | ":="
      {
      WriteToken :: (scanner lexbuf)
    }
  | ";"
      {
      SequenceToken :: (scanner lexbuf)
    }
  | "->"
      {
